...
const faqs: FAQItem[] = [
  ...
  { q: "What sizes are available?", a: "We offer four size categories: Extra Small (6 m²), Small (9 m²), Medium (18 m²), and Large (27 m²). Within each category, we have different unit options. Visit our Units page for full details and pricing.", links: [{ text: "Units page", href: "/units" }] },
  ...
  { q: "Can I cancel my booking?", a: "Yes, you can cancel your booking at any time. Refunds are processed according to our cancellation policy. Monthly contracts have a 30-day notice period. Contact us for specific details about your booking.", links: [{ text: "cancellation policy", href: "/refund-policy" }] },
  ...
  { q: "Are there any hidden fees?", a: "No. Our pricing is transparent. The only additional fee is a R150 administrative cancellation fee for monthly contracts when cancelling with less than 30 days' notice. All fees are clearly stated in our Refund & Cancellation Policy.", links: [{ text: "Refund & Cancellation Policy", href: "/refund-policy" }] },
...
function renderAnswer(text: string, links?: { text: string; href: string }[]) {
  if (!links || links.length === 0) return <span>{text}</span>;

  const parts = text.split(new RegExp(links.map((l) => l.text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|"), "g"));
  ...
  return (
    <span>
      {parts.map((part, i) => {
        ...
        return link ? (
          <Link key={i} href={link.href} className="text-[#D4006A] hover:underline inline-flex items-center gap-1">
            {link.text}
        ...
        <div className="max-w-[640px] mx-auto space-y-2">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-md overflow-hidden">
              ...
              {openIndex === i && (
                <div className="px-4 pb-4 text-sm text-[#a09a95] leading-relaxed">
                  {renderAnswer(faq.a, faq.links)}
                </div>
              )}
            </div>
          ))}
